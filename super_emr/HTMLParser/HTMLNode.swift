    ...
    private static func toXPath(selector: String) -> String? {
        func firstMatch(pattern: String)(str: String) -> NSTextCheckingResult? {
            if let regex = try? NSRegularExpression(pattern: pattern, options: .AnchorsMatchLines) {
                if let result = regex.firstMatchInString(str, options: .ReportProgress, range: NSRange(location: 0, length: str.utf8.count)) {
                    return result
                }
            }
            return nil
            ...
            while let result = matchPseudo(str: str) {
                ...
                default:
                    if let sub = matchSubNthChild(str: one) {
                        ...
                        let arg = (0, Int(b)!)
                        xpath += (nth == "nth-child") ? nth_child(0, b: Int(b)!) : nth_last_child(0, b: Int(b)!)
//                        xpath += (nth == "nth-child") ? nth_child(arg) : nth_last_child(arg)
                        ...
                        let arg = (Int(a)!, Int(b)!)
                        xpath += (nth == "nth-child") ? nth_child(Int(a)!, b: Int(b)!) : nth_last_child(Int(a)!, b: Int(b)!)
//                        xpath += (nth == "nth-child") ? nth_child(arg) : nth_last_child(arg)
