...
class defalutTableViewCell: UITableViewCell {
    ...
    func setData(name:String,imagePic:String,content:String,imgData:[String],indexRow:NSIndexPath,selectItem:Bool,like:[String],likeItem:Bool,CommentNameArray:[String],CommentArray:[String],comments:NSArray){
        var h = cellHeightByData(content)
        let h1 = cellHeightByData1(imgData.count)
        var h2:CGFloat = 0.0
        nameLabel.text = name
        
        let url = NSURL(string:imagePic)
        let data = NSData(contentsOfURL:url!)
        avatorImage.image = UIImage(data:data!)
        if h<13*5{
...
//        }
        ...
        if comments.count>0{
            ...
            self.timeLabel.text = comments[0]["time"] as! String
            for i in 0..<comments.count{
                let comment_view = CommentView()
                comment_view.frame = CGRectMake(55,h3+(CGFloat(i*20)),UIScreen.mainScreen().bounds.width - 10 - 55 - 15,20)
                comment_view.nameLabel.text = comments[i]["uid_name"] as! String
                comment_view.commentLabel.text = comments[i]["ctext"] as! String
                self.contentView.addSubview(comment_view)
            }
        }
