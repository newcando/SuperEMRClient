...
class chatViewControllerPatient: UIViewController, UITableViewDelegate, UITableViewDataSource,UIScrollViewDelegate,UITextFieldDelegate {
...
//        }
    ...
    func contentValue(){
        ...
        Alamofire.request(.GET, tableUrl, parameters: params)
            ...
            .responseJSON {response in
                ...
                self.view.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(chatViewControllerPatient.handleTap(_:))))
                NSNotificationCenter.defaultCenter().addObserver(self, selector:#selector(chatViewControllerPatient.keyBoardWillShow(_:)), name:UIKeyboardWillShowNotification, object: nil)
                NSNotificationCenter.defaultCenter().addObserver(self, selector:#selector(chatViewControllerPatient.keyBoardWillHide(_:)), name:UIKeyboardWillHideNotification, object: nil)
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "咨询", style: UIBarButtonItemStyle.Plain, target: self, action: "zixun")
    ...
    func zixun() {
        self.iscomment = 0
        let xNSNumber = self.user as NSNumber
        let xUserString : String = xNSNumber.stringValue
        
        let chatWith = RCConversationViewController(conversationType: RCConversationType.ConversationType_PRIVATE, targetId: xUserString)
        chatWith.title = self.username
        ...
        -> UITableViewCell
    {
        ...
        cell!.commentchange = { _ in
            ...
            self.commentView.sendBtn.addTarget(self, action: #selector(chatViewControllerPatient.sendComment(_:)), forControlEvents:.TouchUpInside)
    ...
    func sendComment(sender:UIButton){
        //self.goodComm[sender.tag]["commentName"]!.append("胖大海")
        //self.goodComm[sender.tag]["comment"]!.append(commentView.commentTextField.text!)
        let userNSNumber = self.loginUser as NSNumber
        let userString : String = userNSNumber.stringValue
