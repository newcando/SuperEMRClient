...
class xuetangViewController: UIViewController {
    ...
    func setLineChart(){
        Alamofire.request(.GET, self.url + "/api/bloodglucose/", parameters: nil)
            .validate()
            .responseJSON {response in
                let jsonData = response.result.value as! Dictionary<String,AnyObject>
                let results = jsonData["results"] as! [Dictionary<String,AnyObject>]
                let count = jsonData["count"] as! Int
                self.gluocose.removeAll()
                self.measure_datetime.removeAll()
                for(var i = 0;i < count; i++){
                    self.gluocose.append(results[i]["gluocose"] as! Double)
